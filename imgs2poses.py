...
def save_poses(basedir, poses, pts3d, perm):
    ...
    # Create a mapping from COLMAP's image_id to our sequential 0-based array index
    # We use 'perm' because that's the list of images COLMAP actually successfully registered
    id_to_index = {im_id: i for i, im_id in enumerate(perm)}

    for k in pts3d:
        pts_arr.append(pts3d[k].xyz)
        cams = [0] * poses.shape[-1]
        for ind in pts3d[k].image_ids:
            # Only track the point if the image was actually registered
            if ind - 1 in id_to_index: 
                array_index = id_to_index[ind - 1]
                cams[array_index] = 1
        vis_arr.append(cams)
    ...
    for i in perm:
        ...
        # --- FIX: Handle images that see 0 points ---
        if len(zs) == 0:
            print(f"Warning: Image index {i} sees 0 points. Using global scene depth bounds.")
            # Fall back to the valid_z global array calculated just above this loop
            close_depth, inf_depth = np.percentile(valid_z, .1), np.percentile(valid_z, 99.9)
        else:
            close_depth, inf_depth = np.percentile(zs, .1), np.percentile(zs, 99.9)
        # --------------------------------------------
